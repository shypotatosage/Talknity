...
class PostViewModel @Inject constructor(
    ...
    val _ownedComments: MutableLiveData<List<Comment>> by lazy {
        MutableLiveData<List<Comment>>()
    }

    val ownedComments: LiveData<List<Comment>>
        get() = _ownedComments

    val _ownedCommentsError: MutableLiveData<String> by lazy {
        MutableLiveData<String>()
    }

    val ownedCommentsError: LiveData<String>
        get() = _ownedCommentsError

    fun getOwnedComments(uid: String) = viewModelScope.launch {
        repo.getOwnedComments(uid).let { response ->
            if (response.isSuccessful) {
                if (response.body()?.get("message")?.asString == "Success") {
                    _ownedCommentsError.value = "Get Data Successful"
                    _ownedComments.value = arrayListOf()

                    var tmpArrList = arrayListOf<Comment>()

                    if (!response.body()!!.get("data").isJsonNull) {
                        var comments: JsonArray = response.body()!!.getAsJsonArray("data")
                    ...
                    _ownedComments.value = tmpArrList

                    Log.e("Owned Comment Data", _ownedComments.value.toString())
                } else {
                    _ownedCommentsError.value = response.message()
                }

                Log.d("Get Owned Comment Data", _ownedPostsError.value.toString())
            } else {
                Log.e("Get Owned Comment Data Error", response.message())
            }
        }
    }

    val _commentUpdate: MutableLiveData<String> by lazy {
        MutableLiveData<String>()
    }

    val commentUpdate: LiveData<String>
        get() = _commentUpdate
    ...
    fun updateComment(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful) {
                        _commentUpdate.value = "Comment Successfully Updated"
                    } else {
                        _commentUpdate.value = response.message()
                    }

                    Log.d("Update Comment", response.message())
                }
            } catch (e: HttpException) {
                e.printStackTrace()
            } catch (e: Throwable) {
    ...
    fun resetCommentUpdate() {
        _commentUpdate.value = ""
    }

    val _commentDelete: MutableLiveData<String> by lazy {
        MutableLiveData<String>()
    }

    val commentDelete: LiveData<String>
        get() = _commentDelete

    fun deleteComment(id: String) = viewModelScope.launch {
        repo.deleteComment(id).let { response ->
            if (response.isSuccessful) {
                if (response.body()?.get("message")?.asString == "Success") {
                    _commentDelete.value = "Comment Successfully Deleted"
                } else {
                    _commentDelete.value = response.message()
                }

                Log.d(
                    "Delete Comment",
                    _commentDelete.value.toString()
                )
            } else {
                _commentDelete.value = response.message()
                Log.e("Delete Comment", response.raw().message)
            }
        }
    }

    fun resetCommentDelete() {
        _commentDelete.value = ""
    }
}